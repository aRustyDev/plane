...
#
# woven: Generic OIDC SSO views (Open-EE, feature: OIDC/SAML SSO). Public "spaces" portal
# initiate + callback endpoints, mirroring the CE Google/Gitea OAuth space views. Not
# derived from Plane's Commercial/EE OIDC code.
...
class OIDCCallbackSpaceEndpoint(View):
    def get(self, request):
        ...
        # woven: intentionally NOT `base_host = request.session.get("host")`. The CE
        # google/github/gitlab space callbacks bind that local (a str) and then call
        # base_host(request=..., is_space=True) as a function, which raises at runtime.
        # We use the imported base_host() function consistently instead.
